...
def knapsack_greedy_heurestic(objects, knapsack_capacity):
    knapsack = Knapsack(knapsack_capacity)
    sorted_by_profit = sorted(map_profit(objects), key = lambda x: x[1], reverse = True)
    for (val, weight), _ in sorted_by_profit:
        if val < 0:
            break
        if knapsack.taken_capacity + weight > knapsack.capacity:
            continue
        knapsack.taken_capacity += weight
        knapsack.objects.append((val, weight))
        knapsack.value += val

    mapped = "".join(["1" if object_ in knapsack.objects else "0" for object_ in objects])
    codea = int(mapped[:len(objects)//2], 2)
    codeb = int(mapped[len(objects)//2:], 2)
    return ([codea, codeb], knapsack.value, knapsack.taken_capacity)
...
def knapsack_fastforced(objects, knapsack_capacity): 
    best = knapsack_greedy_heurestic(objects, knapsack_capacity)
